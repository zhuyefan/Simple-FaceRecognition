...
my_faces_path = "D:/photos/myself"
other_faces_path = "D:/photos/others"
size = 64
...
def readData(path , h=size, w=size):
    for filename in os.listdir(path): # 返回一个包含文件名的列表(该文件路径下)
        if filename.endswith('.jpg'):
            ...
            # 将图片放大， 扩充图片边缘部分
            # 该函数将源图像复制到目标图像的中间。这些地区左，右，上面和下面的复制源图像将被外推填充
...
# 将图片数据与标签转换成数组
imgs = np.array(imgs) #  将list转换为np.ndarry
labs = np.array([[0,1] if lab == my_faces_path else [1,0] for lab in labs]) # 将label 均换为one-hot编码格式
# 随机划分测试集与训练集
...
def cnnTrain():
    ...
    # 比较标签（索引）是否相等，再求的所有数的平均值，tf.cast(强制转换类型)
    accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(out, 1), tf.argmax(y_, 1)), tf.float32))
    # 将loss与accuracy保存以供tensorboard使用
    tf.summary.scalar('loss', cross_entropy)
    tf.summary.scalar('accuracy', accuracy)
    merged_summary_op = tf.summary.merge_all() # merge_all 可以将所有summary全部保存到磁盘，以便tensorboard显示。如果没有特殊要求，一般
                                                  # 用这一句就可一显示训练时的各种信息了。
